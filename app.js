      ...
      sessionStore.get(sessionID,function(err,session){
            ...
            socket.on('join',(data)=>{
              ...
              const { error, user } = addUser({ id: socket.id,username:eamil,room:data.room,avatar:avatar })
                ...
                socket.emit('message' ,{user:"admin",msg:`${eamil} ,wellcome`}) ;
                socket.broadcast.to(data.room).emit('message',{user:"admin",msg:`${eamil} has just joined`})
            ...
            socket.on('sendMessage', (message, callback) => {
              ...
              io.to(roomData.room).emit('message', {user:eamil,msg: message,avatar:avatar})
              callback()
          })
             socket.on('disconnect',()=>{
              ...
              console.log(user)
               if(user) {
                io.to(roomData.room).emit('message', {user:"admin",msg:`${eamil } has just left`})
                io.to(roomData.room).emit('roomData', {
                  room: user.room,
                  users: getUsersInRoom(user.room)
              })
               }
